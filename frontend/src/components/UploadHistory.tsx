...
import { Eye, Trash2, FileText, Image, Calendar, Search, Filter, ChevronDown } from 'lucide-react';
...
export const UploadHistory: React.FC = () => {
  ...
  const [selectedFile, setSelectedFile] = useState<UploadedFile | ReceiptImage | null>(null);
  ...
  const deleteFile = async (fileId: number, type: 'statement' | 'receipt') => {
    if (!confirm('Are you sure you want to delete this file? This will also delete associated transactions.')) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-teal-600 rounded-2xl p-6 text-white shadow-lg">
        <div>
          <h1 className="text-3xl lg:text-4xl font-bold mb-2">📂 Upload History</h1>
          <p className="text-blue-100 text-lg">View and manage your uploaded statements and receipt images</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          {filteredFiles.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-xl font-medium text-gray-900 mb-2">No Bank Statements</h3>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredFiles.map((file) => (
                    <tr key={file.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${getStatusColor(file.status)}`}>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          {filteredReceipts.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-xl font-medium text-gray-900 mb-2">No Receipt Images</h3>
              <p className="text-gray-600 text-lg">
                {searchQuery 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
              {filteredReceipts.map((receipt) => (
                <div key={receipt.id} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-1">
                      {receipt.merchant_name && (
                        <p className="text-sm text-gray-700">
                          <span className="font-medium">Merchant:</span> {receipt.merchant_name}
                        </p>
                      )}
                      {receipt.amount && (
                        <p className="text-sm font-medium text-gray-900">
                          <span className="text-gray-700">Amount:</span> {formatCurrency(receipt.amount, authState.user)}
                        </p>
                      )}
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${getStatusColor(receipt.processed ? 'completed' : 'processing')}`}>
      ...
      {/* Preview Modal */}
      <Modal isOpen={showPreviewModal} onClose={() => setShowPreviewModal(false)} title="Preview File">
        {selectedFile && (
          <div className="space-y-4">
            ...
            <div className="text-sm text-gray-600">
              ...
              {(selectedFile as ReceiptImage).amount && (
                <p>Amount: {formatCurrency((selectedFile as ReceiptImage).amount!, authState.user)}</p>
              )}
            </div>
          </div>
        )}
