...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for addon_data in addons_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created add-on: {addon.name}')
                )
            else:
                ...
                addon.save()
                self.stdout.write(
                    self.style.WARNING(f'Updated add-on: {addon.name}')
                )
        ...
        try:
            ...
            for addon_name in premium_compatible:
                if addon_name in created_addons:
                    created_addons[addon_name].compatible_plans.add(premium_plan, enterprise_plan)
            
        except SubscriptionPlan.DoesNotExist:
            self.stdout.write(
                self.style.WARNING('Subscription plans not found. Run setup_subscription_plans first.')
            )
        ...
        for template_data in templates_data:
            try:
                base_plan = SubscriptionPlan.objects.get(plan_type=template_data['base_plan'])
                ...
                self.stdout.write(
                    self.style.ERROR(f'Base plan not found for template: {template_data["name"]}')
                )
